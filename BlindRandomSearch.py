...
def blind_random_search(filename):
    initial_runner = create_initial_runner(filename).run()
    best_objective = initial_runner["objective"]
    ...
    for i in range(iterations):
        ...
        current_runner= create_random_runner(filename).run()

        if current_runner["feasible"]:
            current_objective = current_runner["objective"]
    ...
    return best_objective, total_time, initial_runner["objective"]

objectives = []
times = []

for i in range(10):
    obj, tot_time, init_obj = blind_random_search(filename)
